...
const createSendToken = (user, statusCode, res) => {
  ...
  // Sending jwt via http-only-cookie
  const cookieOptions = {
    expires: new Date(
  ...
  // Remove the password from output, so client won't see that
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    user.passwordResetExpires = 'Expired';
    await user.save({ validateBeforeSave: false });
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 3) Update password changedAt property for the user in userModel.js

  // 4) Log the user in, send JWT web token to the client
  createSendToken(user, 200, res);
});
